...
from dashboard.models import TipoTarea, Fase, TareaComun, TareaTareaComun, Tarea
...
def lista_tipos_tarea(request):
    ...
    return render(
        request, "gestion_tareas/configuracion/tipos_tarea/lista.html", context
    )
...
def editar_tipo_tarea(request, id):
    ...
    return render(
        request, "gestion_tareas/configuracion/tipos_tarea/editar.html", context
    )
    ...
    if request.method == "POST":
        try:
            ...
            if Tarea.objects.filter(tipo_tarea=tipo_tarea).exists():
                messages.error(
                    request,
                    "No se puede eliminar el tipo de tarea porque hay tareas asociadas",
                )
                return redirect("gestion_tareas:lista_tipos_tarea")
    ...
    if request.method == "POST":
        try:
            ...
            if Tarea.objects.filter(fase=fase).exists():
                messages.error(
                    request, "No se puede eliminar la fase porque hay tareas asociadas"
                )
                return redirect("gestion_tareas:lista_fases")
            ...
            with transaction.atomic():
                ...
                for i, fase_posterior in enumerate(
                    fases_posteriores, start=orden_eliminado
                ):
                    fase_posterior.orden = i
                    fase_posterior.save()

            messages.success(
                request, "Fase eliminada exitosamente y orden de fases reorganizado"
            )
...
def lista_tareas_comunes(request):
    ...
    return render(
        request, "gestion_tareas/configuracion/tareas_comunes/lista.html", context
    )
...
def crear_tarea_comun(request):
    ...
    return render(
        request, "gestion_tareas/configuracion/tareas_comunes/crear.html", context
    )
...
def editar_tarea_comun(request, id):
    ...
    return render(
        request, "gestion_tareas/configuracion/tareas_comunes/editar.html", context
    )
    ...
    if request.method == "POST":
        try:
            ...
            if TareaTareaComun.objects.filter(idtareacomun=tarea_comun).exists():
                messages.error(
                    request,
                    "No se puede eliminar la tarea común porque hay tareas asociadas",
                )
                return redirect("gestion_tareas:lista_tareas_comunes")
