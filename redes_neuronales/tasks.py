...
from redes_neuronales.ipc_utils import (
    get_queue_for_training,
    clear_queue_for_training,
    send_update,
)
...
class AsyncTask:
    ...
    def _process_wrapper(self, task_id, args, kwargs):
        ...
        try:
            ...
            trace_log(
                f"Proceso hijo iniciado con PID={os.getpid()} para task_id={task_id}",
            ...
            if task_id in _ACTIVE_TASKS:
                ...
                trace_log(
                    f"Task {task_id} marcada como RUNNING en _ACTIVE_TASKS",
                    category="PROCESS_STATE",
                )
            else:
                trace_log(
                    f"¡ADVERTENCIA! Task {task_id} no encontrada en _ACTIVE_TASKS",
            ...
            if not self.target_func:
                raise ValueError(
                    f"No se ha definido una función objetivo para la tarea {task_id}"
                )

            trace_log(
                f"Ejecutando función {self.target_func.__name__} con args={args}",
            ...
            trace_log(
                f"Error en proceso {task_id}: {str(e)}\n{error_trace}",
    ...
    def delay(self, *args, **kwargs):
        ...
        if not self.target_func:
            raise ValueError(
                "No se ha especificado una función objetivo para esta tarea"
            )
        ...
        trace_log(
            f"Creando nueva tarea asíncrona: id={task_id}, nombre={self.name}, función={self.target_func.__name__}",
        ...
        if process.is_alive():
            trace_log(
                f"Proceso iniciado con éxito para task_id={task_id}, PID={process.pid}",
                category="TASK_START_SUCCESS",
            )
        else:
            trace_log(
                f"ADVERTENCIA: El proceso parece no haber iniciado para task_id={task_id}",
...
class AsyncResult:
    ...
    def get(self, timeout=None, propagate=True):
        ...
        if self.process.exitcode != 0 and propagate:
            raise RuntimeError(
                f"La tarea falló con código de salida {self.process.exitcode}"
            )
