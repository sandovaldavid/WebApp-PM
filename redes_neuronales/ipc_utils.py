...
if USE_FALLBACK:
    ...
    _fallback_queues = (
        {}
    )  # Usaremos Queue del módulo queue en lugar de multiprocessing.Queue
...
def get_queue_for_training(training_id):
    ...
    if USE_FALLBACK:
        ...
        try:
            ...
            if DEBUG_IPC:
                print(
                    f"[IPC-Fallback] Nueva cola creada para training_id={training_id}"
                )
            return q
        except Exception as e:
    ...
    try:
        ...
        if DEBUG_IPC:
            print(
                f"[IPC] Nueva cola creada para training_id={training_id} ({id(queue)})"
            )
        return queue
    except Exception as e:
    ...
    if training_id in _queues:
        ...
        while not queue.empty():
            try:
                ...
                break
        debug_trace(
            f"Eliminados {items_removed} elementos de la cola para training_id={training_id}"
        )
    ...
    try:
        ...
        if config:
            ...
            if update_data.get("is_epoch_log") and update_data.get("epoch_number"):
                ...
                cache.set(
                    f"epoch_log_{training_id}_{epoch_num}", update_data, 3600  # 1 hora
                )
    except Exception as e:
            ...
            if queue:
                try:
                    status[training_id] = {
                        ...
                        "empty": (
                            queue.empty() if hasattr(queue, "empty") else "unknown"
                        ),
                    }
                except:
                    status[training_id] = {
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if "handle is closed" in str(e) or "Bad file descriptor" in str(
                            e
                        ):
                            is_closed = True
                        else:
                    ...
                    if not is_closed:
                        ...
                        while (
                            not queue.empty()
                            and (time.time() - start_time) < max_timeout
                        ):
                            try:
                                queue.get_nowait()
                            except Exception as e:
                    ...
                    print(
                        f"[IPC] No se pudo verificar/vaciar la cola: {str(empty_error)}"
                    )

                # Cerrar la cola de manera segura
                try:
                    # Primero intenta join_thread que es más seguro
                    if hasattr(queue, "join_thread") and callable(
                        getattr(queue, "join_thread")
                    ):
                        try:
                            queue.join_thread()
                            print(
                                f"[IPC] join_thread() ejecutado para cola {training_id}"
                            )
                    ...
                    print(
                        f"[IPC] Error durante el proceso de cierre: {str(close_process_error)}"
                    )
                ...
                try:
                    del _queues[training_id]
                    print(
                        f"[IPC] Cola para training_id={training_id} eliminada del diccionario"
                    )
                    return True
                except Exception as del_error:
                    print(
                        f"[IPC] Error al eliminar cola del diccionario: {str(del_error)}"
                    )
                    return False
            except Exception as e:
...
def clean_up_queues():
    ...
    print(
        f"[IPC] Limpieza completa. Quedan {queue_count} colas en el diccionario y {len(_all_queues)} en la lista de colas."
    )
