...
def log_ipc_operation(operation_type, training_id, data=None):
    ...
    if (
        ...
        and data.get("type") == "log"
        and data.get("is_epoch_log")
    ):
        ...
        trace_log(
            f"IPC {operation_type.upper()}: training_id={training_id}", category="IPC"
        )
...
def check_cache_state(training_id):
    ...
    try:
        ...
        if not config:
            return {"present": False, "message": "La configuración no está en caché"}
        ...
        if result["has_updates"]:
            ...
            epoch_logs = [
                u for u in updates if u.get("type") == "log" and u.get("is_epoch_log")
            ]
...
def verify_connections(training_id):
    ...
    """
    result = {"redis_ok": False, "ipc_ok": False, "cache_present": False, "issues": []}
        ...
        try:
            ...
            if config:
                ...
                result["issues"].append(
                    "No se encontró configuración en caché para este entrenamiento"
                )
        except Exception as e:
        ...
        try:
            ...
            if queue:
                ...
                if hasattr(queue, "qsize"):
                    ...
                    if queue_size > 100:
                        result["issues"].append(
                            f"Cola IPC es muy grande ({queue_size} elementos)"
                        )
                else:
                    result["issues"].append(
                        "Cola no tiene método qsize() - posible tipo incorrecto"
                    )
            else:
