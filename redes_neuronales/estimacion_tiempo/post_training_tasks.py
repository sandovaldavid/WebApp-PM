...
matplotlib.rcParams["figure.max_open_warning"] = (
    0  # Evitar advertencias por exceso de figuras
)
        ...
        if config:
            ...
            if success:
                    ...
                    try:
                        ...
                        if os.path.exists(metrics_path):
                            with open(metrics_path, "r") as f:
                                ...
                                print(
                                    f"[PostTraining] Métricas cargadas del archivo: {result_metrics}"
                                )
                    except Exception as e:
                        print(
                            f"[PostTraining] Error al cargar métricas desde archivo: {e}"
                        )
                ...
                for key, value in result_metrics.items():
                    ...
                    normalized_metrics[normalized_key] = (
                ...
                config["updates"].append(
                    {
                        ...
                        "timestamp": time.time()
                        + 0.1,  # Añadir 0.1s para asegurar orden
                        "model_id": training_id,
                        "training_id": training_id,
                        "metrics": normalized_metrics,
                    }
...
def start_background_tasks(training_id=None):
    ...
    thread = Thread(target=run_evaluation_in_background, args=(training_id,))
    # No marcar el hilo como daemon para evitar problemas
    thread.daemon = False
    thread.start()

    print(
        f"[PostTraining] Tarea posterior al entrenamiento iniciada en hilo {thread.name}"
    )
    return thread
