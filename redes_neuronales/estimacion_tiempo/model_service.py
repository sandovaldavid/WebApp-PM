...
from dashboard.models import (
    Tarea, TipoTarea, Fase, Resultadosrnn, Modeloestimacionrnn, 
    Tarearecurso, Recurso, Recursohumano, Requerimiento, HistorialEquipo
)
...
class EstimacionTiempoService:
    ...
    def __init__(self):
        ...
        self.model_path = os.path.join(settings.BASE_DIR, 'redes_neuronales', 'estimacion_tiempo', 'models')
    ...
    def extract_task_features(self, tarea_id):
        ...
        try:
            ...
            if equipo_id:
                historial = HistorialEquipo.objects.filter(idequipo_id=equipo_id).order_by('-fecha_registro').first()
                if historial and historial.completitud_tareas is not None:
                    # Convertir la completitud a una escala de 1-5
                    experiencia_equipo = max(1, min(5, int(historial.completitud_tareas * 5 + 0.5)))
            ...
            logger.error(f"Error al extraer características de la tarea {tarea_id}: {str(e)}")
            return None
    ...
    def estimate_and_save(self, tarea_id):
        ...
        if not success:
            return False, estimated_time, "La estimación se realizó pero no se pudo guardar"
    ...
    def estimate_project_completion(self, proyecto_id):
        ...
        try:
            ...
            tiempo_estimado_total = tareas.aggregate(Sum('duracionestimada'))['duracionestimada__sum'] or 0
            tiempo_actual_total = tareas.aggregate(Sum('duracionactual'))['duracionactual__sum'] or 0
            
            # Calcular porcentaje de completitud
            porcentaje_completitud = (tareas_completadas / total_tareas * 100) if total_tareas > 0 else 0
            
            # Calcular fecha estimada de finalización
            import datetime
            proyecto = tareas.first().idrequerimiento.idproyecto if tareas.exists() else None
            ...
            if proyecto and proyecto.fechainicio:
                ...
                fecha_fin_estimada = proyecto.fechainicio + datetime.timedelta(days=dias_necesarios)
            ...
            logger.error(f"Error al estimar completitud del proyecto {proyecto_id}: {str(e)}")
            return None
    ...
    def reestimate_after_changes(self, tarea_id):
        ...
        try:
            ...
            resultados_previos = Resultadosrnn.objects.filter(
                idtarea_id=tarea_id
            ).order_by('-timestamp').first()
            
            estimacion_previa = resultados_previos.duracionestimada if resultados_previos else 0
            ...
            if estimacion_previa > 0:
                diferencia_porcentual = ((nueva_estimacion - estimacion_previa) / estimacion_previa) * 100
            else:
            ...
            return {
                'status': 'error',
                'message': f"Error en reestimación: {str(e)}"
            }
