...
def load_model_and_preprocessors():
    ...
    try:
        ...
        if not os.path.exists(feature_dims_path):
            raise FileNotFoundError(f"No se encontró feature_dims.pkl en {feature_dims_path}")
        ...
        if os.path.exists(X_val_path) and os.path.exists(y_val_path):
            ...
            print(f"Datos de validación cargados: X_val: {X_val.shape}, y_val: {y_val.shape}")
        else:
            ...
            if os.path.exists(joblib_path):
                try:
                    ...
                    if X_val is not None and y_val is not None:
                        print(f"Datos de validación cargados desde joblib: X_val: {X_val.shape}, y_val: {y_val.shape}")
                except Exception as e:
                    print(f"Error al cargar datos desde joblib: {e}")
        
        # 3. Si aún no hay datos, generar datos sintéticos para evaluación
        if X_val is None or y_val is None:
            print("No se encontraron datos de validación. Generando datos sintéticos para evaluación...")
            total_dims = sum(feature_dims.values())
            X_val = np.random.randn(100, total_dims) * 0.5 + 0.5  # Valores positivos cercanos a 0.5
            y_val = np.abs(np.random.randn(100) * 10 + 20)  # Valores positivos con media 20
            
            # Guardar estos datos para uso futuro
            np.save(X_val_path, X_val)
            np.save(y_val_path, y_val)
            print(f"Datos sintéticos generados: X_val: {X_val.shape}, y_val: {y_val.shape}")
...
def generate_feature_importance(X_val, y_val, estimator, feature_dims):
    ...
    try:
        ...
        if sum_importance > 0:
            feature_importance_df['Importance_Normalized'] = feature_importance_df['Importance'] / sum_importance
        else:
            feature_importance_df['Importance_Normalized'] = 0
        
        # Guardar CSV
        feature_importance_path = os.path.join(MODELS_DIR, 'global_feature_importance.csv')
            ...
            if len(valores_unicos) >= 3:
                ...
                for nombre, mascara in segmentos.items():
                    ...
                    for i, feature_name in enumerate(feature_names):
                        ...
                        base_error_segment = mean_squared_error(y_segment, y_base_segment)
                        
                        y_perturbed_segment = estimator.predict(X_perturbed, feature_dims)
                        perturbed_error_segment = mean_squared_error(y_segment, y_perturbed_segment)
                    ...
                    segment_df = pd.DataFrame(segment_importance_values, columns=['Feature', 'Importance'])
                    ...
                    segment_path = os.path.join(MODELS_DIR, f'feature_importance_{nombre}.csv')
                    segment_df.to_csv(segment_path, index=False)
                    print(f"Guardado análisis para {nombre}: {segment_path}")
            else:
                print("No hay suficientes valores únicos para crear segmentos por número de recursos")
        except Exception as e:
...
def update_metrics_history(metrics):
    ...
    try:
        ...
        if isinstance(history, list) and len(history) > 20:
            history = history[-20:]
        elif isinstance(history, dict) and 'entries' in history and len(history['entries']) > 20:
