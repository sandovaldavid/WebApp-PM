...
class AdvancedRNNEstimator:
    ...
    def _get_optimizer(self, optimizer_name):
        ...
        if optimizer_name == "adam":
            ...
            print(
                f"Optimizador '{optimizer_name}' no reconocido. Usando Adam predeterminado."
            )
    ...
    def _create_tf_dataset(
        self, X, y, feature_dims, batch_size=None, shuffle=True, cache=True
    ):
    ...
    def train(
        ...
        default_callbacks = [
            ...
            ModelCheckpoint(
                filepath=(
                    "./checkpoints/model_{epoch:02d}_{val_loss:.4f}.keras"
                    if validation_data
                    else "./checkpoints/model_{epoch:02d}_{loss:.4f}.keras"
                ),
                save_best_only=True,
                monitor="val_loss" if validation_data else "loss",
                verbose=1,
        ...
        print(
            f"Iniciando entrenamiento con {self.config['batch_size']} muestras por batch"
        )
        print(
            f"Optimizador: {self.config['optimizer']}, Learning rate: {self.config['learning_rate']}"
        )
        ...
        if self.history is not None:
            ...
            if "val_loss" in self.history.history:
                history_dict["val_loss"] = [
                    float(val) for val in self.history.history["val_loss"]
                ]
                history_dict["val_mae"] = [
                    float(val) for val in self.history.history["val_mae"]
                ]
                history_dict["val_mape"] = [
                    float(val) for val in self.history.history["val_mape"]
                ]
    ...
    def feature_importance(
        self, X, y, feature_dims, feature_names=None, n_samples=1000
    ):
        ...
        perm_importance = permutation_importance(
            predict_func, X_sample, y_sample, n_repeats=10, random_state=42, n_jobs=-1
        )
