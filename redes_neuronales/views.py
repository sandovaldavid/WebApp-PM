    ...
    if request.method == 'POST' and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        ...
        config = {
            ...
            "early_stopping_patience": int(
                request.POST.get("early_stopping_patience", "30")
            ),
        ...
        cache_success = safe_cache_set(
            f"training_config_{training_id}", config, 7200
        )  # 2 horas de vida
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Iniciando entrenamiento con ID {training_id} y usuario {usuario_id}"
                )
                ...
                task_id = task_manager.start_task(
                    ejecutar_entrenamiento, training_id, usuario_id
                )
...
def _stream_training_updates(training_id, session):
    ...
    last_check_time = (
        time.time()
    )  # Último momento en que se verificaron actualizaciones
    ...
    yield _format_sse_event(
        "connection",
        {"status": "connected", "training_id": training_id, "timestamp": time.time()},
    )
    ...
    while True:
        ...
        try:
            ...
            if status in ["completed", "failed"]:
                ...
                yield _format_sse_event(
                    "close", {"status": status, "message": "Stream finalizado"}
                )
                break
            ...
            if (
                cycles_without_updates > 300
                and current_time - last_diagnostic_time >= diagnostic_interval
            ):
...
def _prepare_complete_event(training_id):
    ...
    if not complete_data["epoch_logs"] and "updates" in config:
        complete_data["epoch_logs"] = [
            ...
            if u.get("is_epoch_log", False)
            or (u.get("type") == "progress" and "epoch" in u)
        ]
    ...
    for field in ["metrics", "history", "predictions", "y_test"]:
        if not complete_data[field] or not isinstance(
            complete_data[field], (dict, list)
        ):
            if field == "metrics":
    ...
    print(
        f"Preparando evento complete para {training_id} - métricas: {complete_data['metrics']}"
    )
    return complete_data
...
def check_active_training(request):
    ...
    try:
        ...
        if not all_keys:
            ...
            for key in all_keys:
                if key.startswith("training_config_"):
                    ...
                    if (
                        training_user_id == usuario_id
                        and config
                        and config.get("status") in ["running", "pending"]
                    ):
                        active_trainings.append(
                            {
                                "training_id": training_id,
        ...
        return JsonResponse({"success": True, "active_trainings": active_trainings})
    except Exception as e:
        import traceback
...
def evaluar_modelo(request):
    ...
    try:
        ...
        models_dir = os.path.join(
            BASE_DIR, "redes_neuronales", "estimacion_tiempo", "models"
        )
        ...
        if not os.path.exists(model_keras_path):
            return JsonResponse(
                {"success": False, "message": "El modelo no existe en el sistema"}
            )
        ...
        try:
            ...
            evaluation_plots_url = copy_and_get_url(
                os.path.join(models_dir, "evaluation_plots.png"), "evaluation_plots"
            )
            ...
            traceback.print_exc()
            return JsonResponse(
                {"success": False, "message": f"Error durante la evaluación: {str(e)}"}
            )
...
def generar_informe_evaluacion(request):
    ...
    try:
        ...
        try:
            ...
            print(
                "Archivo de métricas no encontrado, generando informe con datos limitados"
            )
        ...
        def add_page_number(canvas, doc):
            ...
            canvas.drawString(
                doc.leftMargin,
                doc.height + doc.topMargin - header_height + 15,
            ...
            canvas.line(
                doc.leftMargin, footer_y + 10, doc.width + doc.leftMargin, footer_y + 10
            )
        ...
        elements.append(
            Paragraph(f"Fecha generación: {fecha_generacion}", styles["Normal"])
        )
                ...
                for j in range(2):
                    if i + j < len(main_metrics):
                        ...
                        if (
                            ...
                            if value > 0.85:
                                value_color = colors.Color(
                                    10 / 255, 150 / 255, 80 / 255
                                )  # Verde elegante
                            elif value > 0.7:
                                value_color = colors.Color(
                                    30 / 255, 160 / 255, 90 / 255
                                )  # Verde moderado
                            elif value > 0.5:
                                value_color = colors.Color(
                        ...
                        metric_cell = [
                            ...
                            # Valor
                            Paragraph(
                                f"{value:.4f}" if code != "ACC" else f"{value:.2f}",
                                ParagraphStyle(
                                    "MetricValue",
                                    parent=styles["Normal"],
                                    fontName="Helvetica-Bold",
                            ...
                            TableStyle(
                                [
                                    ...
                                    (
                                        ...
                                        (col, row),
                                        colors.white,
                                    ),  # Asegurar texto claro en fondo oscuro
                                ]
                            )
                        )
        ...
        if include_metrics and metrics:
            elements.append(
                Paragraph("MÉTRICAS DE EVALUACIÓN", styles["CustomSubtitle"])
            )
            ...
            for key, name in main_metrics:
                if key in metrics:
                    value = metrics[key]
                    metric_data.append(
                        [name, f"{value:.4f}", metric_descriptions.get(key, "")]
                    )

            # Otras métricas adicionales
            for key, value in metrics.items():
                if key not in ["MSE", "MAE", "RMSE", "R2"] and isinstance(
                    value, (int, float)
                ):
                    metric_data.append(
                        [key, f"{value:.4f}", metric_descriptions.get(key, "")]
                    )
            ...
            if len(metric_data) > 1:
                ...
                for i in range(1, len(metric_data)):
                    if len(metric_data[i]) > 2:  # Si tiene columna de descripción
                        ...
                        metric_data[i][2] = Paragraph(
                            metric_data[i][2], description_style
                        )

                # 2. Ajustar anchos de columnas - dar más espacio a la descripción
                table = Table(
                    metric_data, colWidths=[2.0 * inch, 0.8 * inch, 3.2 * inch]
                )
                ...
                for i in range(1, len(metric_data)):
                    ...
                    if i % 2 == 1:  # Filas impares
                        table_style.append(
                            ("BACKGROUND", (0, i), (-1, i), colors_palette["light_bg"])
                        )
                    ...
                    table_style.extend(
                        [
                            (
                                ...
                                (0, i),
                                "LEFT",
                            ),  # Nombre de métrica alineado a la izquierda
                            ...
                            (
                                ...
                                (-1, i),
                                "TOP",
                            ),  # Alineación vertical superior para todas las celdas
                            (
                                "FONTNAME",
                                (1, i),
                                (1, i),
                            ...
                            (
                                ...
                                (2, i),
                                10,
                            ),  # Más padding izquierdo para descripción
                            (
                                "RIGHTPADDING",
                                (2, i),
                                (2, i),
                                10,
                            ),  # Más padding derecho para descripción
                        ]
                    )
        ...
        if include_charts:
            ...
            elements.append(
                Paragraph("VISUALIZACIÓN DE RESULTADOS", styles["CustomSubtitle"])
            )
            ...
            evaluation_plots_path = (
                os.path.join(
                    static_dir,
                    [f for f in os.listdir(static_dir) if "evaluation_plots" in f][0],
                )
                if os.path.exists(static_dir)
                and any("evaluation_plots" in f for f in os.listdir(static_dir))
                else None
            )
            ...
            if evaluation_plots_path and os.path.exists(evaluation_plots_path):
                ...
                elements.append(
                    Paragraph("Análisis de Predicciones", styles["Highlight"])
                )
                ...
                img_table = Table(
                    [[img]], colWidths=[6.5 * inch], rowHeights=[4.5 * inch]
                )
                img_table.setStyle(
                    TableStyle(
                        [
        ...
        if include_feature_importance:
            feature_importance_path = (
                os.path.join(
                    static_dir,
                    [f for f in os.listdir(static_dir) if "feature_importance" in f][0],
                )
                if os.path.exists(static_dir)
                and any("feature_importance" in f for f in os.listdir(static_dir))
                else None
            )
            ...
            if feature_importance_path and os.path.exists(feature_importance_path):
                elements.append(PageBreak())
                elements.append(
                    Paragraph("ANÁLISIS DE CARACTERÍSTICAS", styles["CustomSubtitle"])
                )
                ...
                elements.append(
                    Paragraph(
                        "Importancia Relativa de Características", styles["Highlight"]
                    )
                )
                elements.append(Spacer(1, 0.1 * inch))
        ...
        segmented_metrics_path = (
            os.path.join(
                static_dir,
                [f for f in os.listdir(static_dir) if "segmented_metrics" in f][0],
            )
            if os.path.exists(static_dir)
            and any("segmented_metrics" in f for f in os.listdir(static_dir))
            else None
        )
        ...
        if segmented_metrics_path and os.path.exists(segmented_metrics_path):
            elements.append(PageBreak())
            elements.append(
                Paragraph("ANÁLISIS POR SEGMENTOS", styles["CustomSubtitle"])
            )
            ...
            # Título de la sección
            elements.append(
                Paragraph("Rendimiento por Categorías", styles["Highlight"])
            )
            elements.append(Spacer(1, 0.1 * inch))
        ...
        def add_footer(canvas, doc):
            ...
            canvas.setStrokeColor(colors_palette["border"])
            canvas.line(
                doc.leftMargin, 0.5 * inch, doc.width + doc.leftMargin, 0.5 * inch
            )
            ...
            canvas.drawCentredString(
                doc.width / 2 + doc.leftMargin, 0.3 * inch, f"Página {page_num}"
            )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="evaluacion_modelo_{model_id[:8]}.pdf"'
        )
        response.write(pdf)
        ...
        traceback.print_exc()
        return JsonResponse(
            {"success": False, "message": f"Error al generar el informe: {str(e)}"}
        )
...
def diagnosticar_entrenamiento(request):
    ...
    if not training_id:
        return JsonResponse(
            {"success": False, "error": "ID de entrenamiento no proporcionado"},
            status=400,
        )
    ...
    try:
        ...
        last_updates = get_last_updates(
            training_id, 10
        )  # Obtener las últimas 10 actualizaciones
        ...
        trace_log(
            f"Diagnóstico completo para training_id={training_id}",
...
def diagnosticar_logs_epoca(training_id, request=None):
    ...
    result = {
        "total_found": 0,
        "in_cache": {"count": 0, "epochs_found": []},
        "in_queue": {"count": 0, "queue_size": 0, "epochs_found": []},
        "issues": [],
        "process_info": {},
    }
    ...
    try:
        ...
        if config:
            ...
            epoch_logs = [
                u
                for u in updates
                if u.get("type") == "log" and (u.get("is_epoch_log") or "epoch" in u)
            ]
            ...
            for log in epoch_logs:
                ...
                if (
                    epoch_num
                    and isinstance(epoch_num, (int, str))
                    and str(epoch_num).isdigit()
                ):
        ...
        result["connection_status"] = {
            ...
            "ipc_queue_available": (
                hasattr(queue, "qsize") if "queue" in locals() else False
            ),
        ...
        error_trace = traceback.format_exc()
        trace_log(
            f"Error en diagnóstico de logs de época: {str(e)}\n{error_trace}",
...
def get_last_updates(training_id, limit=10):
    ...
    if config and "updates" in config:
        updates = config["updates"][
            -limit:
        ]  # Obtener las últimas 'limit' actualizaciones
...
def open_tensorboard(request):
    ...
    if not model_id:
        return JsonResponse(
            {"success": False, "message": "ID de modelo no proporcionado"}
        )
    ...
    try:
        ...
        traceback.print_exc()
        return JsonResponse(
            {"success": False, "message": f"Error al iniciar TensorBoard: {str(e)}"}
        )
...
def reload_model(request):
    ...
    try:
        ...
        traceback.print_exc()
        return JsonResponse(
            {"success": False, "message": f"Error al recargar el modelo: {str(e)}"},
            status=500,
        )
