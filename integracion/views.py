...
def index(request):
    ...
    context = {"jira_config": jira_config, "integration_stats": stats}
...
def configurar_jira(request):
    ...
    try:
        ...
        if success:
            ...
            return JsonResponse(
                {"success": False, "message": f"Error al conectar con Jira: {message}"},
                status=400,
            )
...
def configuracion_avanzada_jira(request):
    ...
    try:
        ...
        jira_config.importar_issues = data.get(
            "importar_issues", jira_config.importar_issues
        )
        jira_config.exportar_tareas = data.get(
            "exportar_tareas", jira_config.exportar_tareas
        )
        jira_config.sync_comentarios = data.get(
            "sync_comentarios", jira_config.sync_comentarios
        )
        jira_config.sincronizar_adjuntos = data.get(
            "sincronizar_adjuntos", jira_config.sincronizar_adjuntos
        )
        ...
        return JsonResponse(
            {"success": False, "message": "No hay configuración de Jira"}, status=404
        )
...
def listar_proyectos_jira(request):
    ...
    try:
        ...
        jira_projects_data = [
            {"id": p.id, "key": p.key, "name": p.name} for p in jira_projects
        ]
...
def mapear_proyecto(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Proyecto mapeado correctamente"}
        )
...
def sincronizar_jira(request):
    ...
    try:
        ...
        if result:
            ...
            return JsonResponse(
                {"success": False, "message": "Error durante la sincronización"},
                status=500,
            )
        ...
        return JsonResponse(
            {"success": False, "message": f"Error durante la sincronización: {str(e)}"},
            status=500,
        )
...
def listar_usuarios_jira(request):
    ...
    try:
        ...
        try:
            ...
            jira_users = (
                client.get_jira_users()
            )  # Usaremos un método personalizado en JiraClient
            jira_users_data = [
                {
...
def verificar_salud_integracion(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "health_report": health_report})
...
def diagnosticar_campos_jira(request):
    ...
    try:
        ...
        if not jira_config:
            return JsonResponse(
                {"success": False, "message": "No hay configuración de Jira"},
                status=404,
            )
        ...
        return JsonResponse({"success": True, "fields_info": fields_info})
