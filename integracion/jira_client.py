...
class JiraClient:
    def __init__(self, integracion_id=None, config=None):
        if integracion_id:
            try:
                ...
                raise ValueError(
                    f"No existe configuración de Jira con ID {integracion_id}"
                )
        elif config:
            self.config = config
        else:
    ...
    def _connect_to_jira(self):
        ...
        try:
            ...
            return JIRA(
                options, basic_auth=(self.config.usuario_jira, self.config.api_key)
            )
    ...
    def get_issue_comments(self, issue_key):
        ...
        try:
            ...
            logger.error(
                f"Error al obtener comentarios del issue {issue_key}: {str(e)}"
            )
            return []
        ...
        try:
            ...
            try:
                project_mapping = JiraProjectMapping.objects.get(
                    integracion=self.config, proyecto_local_id=proyecto_local.idproyecto
                )
            except JiraProjectMapping.DoesNotExist:
                logger.error(
                    f"No hay mapeo para el proyecto {proyecto_local.idproyecto}"
                )
                return None
            ...
            logger.info(
                f"Tarea {tarea.idtarea} creada exitosamente en Jira como {issue.key}"
            )
            return issue
        except Exception as e:
        ...
        try:
            ...
            try:
                task_mapping = JiraTaskMapping.objects.get(
                    integracion=self.config, tarea_local_id=tarea.idtarea
                )
            ...
            if update_dict:
                ...
                logger.info(
                    f"Campos básicos actualizados para la tarea {tarea.idtarea} en Jira"
                )
    ...
    def _sync_comments_to_jira(self, tarea, jira_issue_key):
        ...
        try:
            ...
            for actividad in activities:
                ...
                comment_text = (
                    f"{actividad.idusuario.nombreusuario}: {actividad.descripcion}"
                )
    ...
    def _update_issue_status(self, issue, target_status):
        ...
        try:
            ...
            logger.info(
                f"Transiciones disponibles para {issue.key}: {available_transitions}"
            )
            ...
            for t in transitions:
                if (
                    ...
                    or t["to"]["name"].lower() == target_status.lower()
                ):
                    transition_id = t["id"]
                    break
            ...
            if transition_id:
                ...
                logger.warning(
                    f"No se encontró transición a estado {target_status} para el issue {issue.key}"
                )
                return False
    ...
    def sync_all_data(self):
        ...
        try:
            ...
            if (
                not hasattr(self.config, "exportar_tareas")
                or self.config.exportar_tareas
            ):
    ...
    def sync_from_jira(self):
        ...
        try:
            ...
            project_mappings = JiraProjectMapping.objects.filter(
                integracion=self.config
            )
            ...
            for mapping in project_mappings:
                logger.info(
                    f"Importando issues del proyecto Jira {mapping.jira_project_key}"
                )

                # Obtener issues de Jira para este proyecto
                issues = self.get_issues(
                    mapping.jira_project_key, updated_since=sync_since
                )

                if not issues:
                    logger.info(
                        f"No hay issues para importar del proyecto {mapping.jira_project_key}"
                    )
                    continue
                ...
                try:
                    proyecto_local = Proyecto.objects.get(
                        idproyecto=mapping.proyecto_local_id
                    )
                except Proyecto.DoesNotExist:
                    logger.error(
                        f"No existe el proyecto local con ID {mapping.proyecto_local_id}"
                    )
                    continue
                ...
                for issue in issues:
                    ...
                    if (
                        hasattr(self.config, "sync_comentarios")
                        and self.config.sync_comentarios
                    ):
                        comentarios_importados = self._import_issue_comments(issue)
                        total_comentarios_procesados += comentarios_importados

            logger.info(
                f"Importación completada: {total_issues_procesados} issues y {total_comentarios_procesados} comentarios procesados"
            )
            return True
    ...
    def _import_issue(self, issue, proyecto_local, requerimiento_default):
        ...
        try:
            ...
            task_mapping = JiraTaskMapping.objects.filter(
                integracion=self.config, jira_issue_id=issue.id
            ).first()
            ...
            datos_tarea = {
                ...
                "estado": self.estado_jira_to_local.get(
                    issue.fields.status.name, "Pendiente"  # Estado por defecto
                ),
            }
            ...
            if hasattr(issue.fields, "priority") and issue.fields.priority:
                datos_tarea["prioridad"] = self.prioridad_jira_to_local.get(
                    issue.fields.priority.name, 3  # Prioridad media por defecto
                )
            ...
            if task_mapping:
                try:
                    ...
                    logger.info(
                        f"Actualizada tarea local ID {tarea_local.idtarea} desde issue {issue.key}"
                    )
                    return tarea_local
            ...
            if not task_mapping:
                ...
                logger.info(
                    f"Creada tarea local ID {tarea_local.idtarea} desde issue {issue.key}"
                )
    ...
    def _import_issue_comments(self, issue):
        ...
        try:
            ...
            if not task_mapping:
                logger.warning(
                    f"No hay tarea local mapeada para el issue {issue.key}, no se importarán comentarios"
                )
                return 0
            ...
            imported_comment_ids = set(
                JiraComentarioMapping.objects.filter(
                    integracion=self.config, jira_issue_key=issue.key
            ...
            try:
                ...
                logger.error(
                    f"No existe la tarea local con ID {task_mapping.tarea_local_id}"
                )
                return 0
            ...
            logger.error(
                f"Error al importar comentarios del issue {issue.key}: {str(e)}"
            )
            return 0
    ...
    def sync_to_jira(self):
        ...
        try:
            ...
            for mapping in project_mappings:
                logger.info(
                    f"Sincronizando proyecto local {mapping.proyecto_local_id} con proyecto Jira {mapping.jira_project_key}"
                )
                ...
                logger.info(
                    f"Requerimientos encontrados para proyecto {mapping.proyecto_local_id}: {requerimientos.count()}"
                )
                ...
                logger.info(
                    f"Encontradas {tareas_count} tareas para sincronizar en el proyecto {mapping.proyecto_local_id}"
                )

                if tareas_count == 0:
                    # Solo registrar advertencia, no detener la sincronización
                    logger.warning(
                        f"No hay tareas para sincronizar en el proyecto {mapping.proyecto_local_id}"
                    )
                ...
                for tarea in tareas:
                    ...
                    if task_mapping:
                        ...
                        if result:
                            ...
                            logger.info(
                                f"Tarea actualizada exitosamente con key {result.key}"
                            )
                        else:
                            logger.error(
                                f"Error al actualizar tarea {tarea.idtarea} en Jira"
                            )
                    else:
                        ...
                        if result:
                            ...
                            logger.info(
                                f"Tarea creada exitosamente con key {result.key}"
                            )
                        else:
                            logger.error(
                                f"Error al crear tarea {tarea.idtarea} en Jira"
                            )

            logger.info(
                f"Exportación completada: {total_tareas_creadas} tareas creadas y {total_tareas_actualizadas} actualizadas"
            )
            return True
    ...
    def _crear_issue_informativo_si_necesario(self, jira_project_key):
        ...
        try:
            ...
            if issues:
                ...
                issue.update(fields=update_dict)
                logger.info(
                    f"Issue informativo actualizado en proyecto {jira_project_key}"
                )
                return issue
            else:
                ...
                issue_dict = {
                    ...
                    "issuetype": {
                        "name": "Task"
                    },  # Usar el tipo adecuado según la instancia de Jira
                    "labels": ["info", "automático", "sin-tareas"],
                }

                issue = self.jira.create_issue(fields=issue_dict)
                logger.info(
                    f"Issue informativo creado en proyecto {jira_project_key} con key {issue.key}"
                )
                return issue

        except Exception as e:
            logger.warning(
                f"No se pudo crear/actualizar el issue informativo en {jira_project_key}: {str(e)}"
            )
            return None
            ...
            for local_user in local_users:
                ...
                for jira_user in jira_users:
                    if (
                        ...
                        and jira_user.emailAddress.lower() == local_user.email.lower()
                    ):
    ...
    def get_project_status(self, project_mapping):
        ...
        try:
            ...
            try:
                proyecto_local = Proyecto.objects.get(
                    idproyecto=project_mapping.proyecto_local_id
                )
            except Proyecto.DoesNotExist:
                return {
            ...
            total_tareas_locales = Tarea.objects.filter(
                idrequerimiento__in=requerimientos
            ).count()
            ...
            try:
                ...
                total_issues_jira = len(
                    self.jira.search_issues(jql, maxResults=1, fields="id")
                )
            except Exception:
            ...
            return {
                ...
                "sincronizacion": {
                    ...
                    "porcentaje_sincronizado": (
                        ...
                        if total_tareas_locales > 0
                        else 0
                    ),
                    "estado": (
                        "Sincronizado"
                        if tareas_mapeadas == total_tareas_locales
                        and total_tareas_locales > 0
                        else "Parcial" if tareas_mapeadas > 0 else "No sincronizado"
                    ),
                },
            }
        except Exception as e:
            logger.error(f"Error al obtener estado del proyecto: {str(e)}")
            return {"error": f"Error al obtener estado del proyecto: {str(e)}"}
    ...
    def clean_orphaned_mappings(self):
        ...
        try:
            ...
            total_deleted = 0
            if orphaned_local:
                deleted = JiraTaskMapping.objects.filter(
                    id__in=orphaned_local
                ).delete()[0]
                total_deleted += deleted
                logger.info(
                    f"Eliminados {deleted} mapeos con tareas locales inexistentes"
                )

            if orphaned_jira:
                deleted = JiraTaskMapping.objects.filter(id__in=orphaned_jira).delete()[
                    0
                ]
                total_deleted += deleted
                logger.info(
                    f"Eliminados {deleted} mapeos con issues de Jira inexistentes"
                )

            # Hacer lo mismo con los comentarios
            orphaned_comments = JiraComentarioMapping.objects.filter(
                integracion=self.config, actividad_local_id__isnull=False
            ).exclude(
                actividad_local_id__in=Actividad.objects.values_list(
                    "idactividad", flat=True
                )
            )
            ...
            return {"success": False, "message": f"Error al limpiar mapeos: {str(e)}"}
    ...
    def check_integration_health(self):
        ...
        try:
            ...
            task_mappings = JiraTaskMapping.objects.filter(
                integracion=self.config
            ).count()
    ...
    def generate_sync_report(self, days_back=30):
        ...
        for mapping in project_mappings:
            ...
            tareas_recientes = Tarea.objects.filter(
                idrequerimiento__in=requerimientos, fechacreacion__gte=start_date
            ).count()
            ...
            projects_data.append(
                {
                    "proyecto_local": {
                        ...
                        "nombre": project_status.get("proyecto_local", {}).get(
                            "nombre", "Desconocido"
                        ),
                    ...
                    "sincronizacion": {
                        ...
                        "porcentaje": project_status.get("sincronizacion", {}).get(
        ...
        return {
            "periodo": {"inicio": start_date, "fin": timezone.now()},
            "estadisticas": {
        ...
        try:
            ...
            for letter in "aeiou":  # Vocales - probablemente aparecen en muchos nombres
                ...
                existing_account_ids = set(
                    u.accountId for u in all_users if hasattr(u, "accountId")
                )
                for user in users:
                    if (
                        not hasattr(user, "accountId")
                        or user.accountId not in existing_account_ids
                    ):
