...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE(f"Buscando actividades redundantes en los últimos {days} días..."))
        ...
        for key, acts in groups.items():
            ...
            for i in range(len(acts) - 1):
                ...
                time_diff = (next_act.fechacreacion - current.fechacreacion).total_seconds()
                
                # Si están muy cerca en tiempo, marcar la segunda como redundante
                if time_diff < window:
                    redundant_ids.append(next_act.idactividad)
                    self.stdout.write(f"  Redundante: ID {next_act.idactividad}, {time_diff:.2f}s después de ID {current.idactividad}")
        
        # Eliminar las actividades redundantes
        if redundant_ids:
            self.stdout.write(self.style.WARNING(f"Se encontraron {len(redundant_ids)} actividades redundantes"))
            
            if not dry_run:
                with transaction.atomic():
                    # Primero eliminar detalles
                    from dashboard.models import DetalleActividad
                    DetalleActividad.objects.filter(idactividad__idactividad__in=redundant_ids).delete()
                    
                    # Luego eliminar las actividades
                    deleted = Actividad.objects.filter(idactividad__in=redundant_ids).delete()
                    self.stdout.write(self.style.SUCCESS(f"Eliminadas {deleted[0]} actividades redundantes"))
            else:
                self.stdout.write(self.style.NOTICE("Modo simulación: No se realizaron cambios"))
        else:
            self.stdout.write(self.style.SUCCESS("No se encontraron actividades redundantes"))
