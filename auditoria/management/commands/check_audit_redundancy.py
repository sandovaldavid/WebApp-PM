        ...
        for key, groups in redundancy_groups.items():
            for group in groups:
                if len(group) > 1:
                    ...
                    for act in group:
                        self.stdout.write(f"  - ID: {act.idactividad}, Fecha: {act.fechacreacion}, Usuario: {act.idusuario.nombreusuario}")
        
        if redundant_count == 0:
            self.stdout.write(self.style.SUCCESS("No se encontraron actividades redundantes"))
        else:
            self.stdout.write(self.style.ERROR(f"Se encontraron {redundant_count} grupos de actividades redundantes"))
