...
def get_model_config(model_instance):
    ...
    try:
        ...
        configs = list(
            ConfiguracionAuditoria.objects.filter(modelo=model_name, campo__isnull=True)
        )
...
def registrar_cambios(instance, changes, action_type, user=None):
    ...
    # NIVEL 2 y NIVEL 3: Registro detallado
        ...
        if action_type == 'MODIFICACION' and num_cambios > 0:
            ...
            campos_str = (
                ...
                if len(campos_modificados) <= 5
                else f"{len(campos_modificados)} campos"
            )
    ...
    if changes and isinstance(changes, dict):
        for field, values in changes.items():
            ...
            field_config = ConfiguracionAuditoria.objects.filter(
                modelo=model_name, campo=field
            ).first()
...
def audit_post_save(sender, instance, created, **kwargs):
    ...
    if (created and config.auditar_crear) or (
        not created and config.auditar_modificar and changes
    ):
