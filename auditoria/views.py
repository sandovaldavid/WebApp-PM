    ...
    if request.method == "POST":
        try:
            ...
            if not nombre or not idusuario or not accion:
                ...
                return render(
                    request, "auditoria/crear_actividad.html", {"usuarios": usuarios}
                )
...
def detalle_actividad(request, id):
    ...
    if nivel_detalle == 3 and actividad.entidad_tipo and actividad.entidad_id:
        entidad_relacionada = obtener_entidad_relacionada(
            actividad.entidad_tipo, actividad.entidad_id
        )
...
def obtener_entidad_relacionada(modelo, id):
    ...
    try:
        ...
        from dashboard.models import (
            ...
            Requerimiento,
            Notificacion,
            Alerta,  # Añadimos estos dos modelos
        )
        ...
        if modelo in model_map:
            ...
            if not pk_field:
                print(
                    f"No se encontró el campo de clave primaria para el modelo {modelo}"
                )
                return None
            ...
            print(
                f"Buscando {modelo} con {pk_field}={id}: {'Encontrado' if instance else 'No encontrado'}"
            )
            return instance
    except Exception as e:
...
def configuracion_auditoria(request):
    ...
    navegacion_config = ConfiguracionGeneralAuditoria.objects.filter(
...
def actualizar_config_navegacion(request):
    ...
    if request.method == "POST":
        try:
            ...
            messages.success(
                request, "Configuración de navegación actualizada con éxito"
            )
        except Exception as e:
...
def configuracion_global_auditoria(request):
    ...
    configuraciones_sugeridas = [
        {
            "nombre": "registrar_navegacion",
            "valor": "true",
            "descripcion": "Determina si se registran las actividades de navegación",
        },
        {
            "nombre": "periodo_retencion_logs",
            "valor": "90",
            "descripcion": "Días que se conservan los logs antes de eliminarlos",
        },
        {
            "nombre": "limite_registros_diarios",
            "valor": "10000",
            "descripcion": "Número máximo de registros de auditoría por día",
        },
        {
            "nombre": "nivel_detalle_global",
            "valor": "2",
            "descripcion": "Nivel de detalle predeterminado para todas las auditorías",
        },
        {
            "nombre": "ips_excluidas",
            "valor": "127.0.0.1,192.168.1.5",
            "descripcion": "IPs que no se auditan (separadas por comas)",
        },
    ]
    ...
    sugerencias_filtradas = [
        ...
        if cfg["nombre"] not in nombres_existentes
    ]
    ...
    if request.method == "POST":
        try:
            ...
            ConfiguracionGeneralAuditoria.objects.create(
                nombre=nombre, valor=valor, descripcion=descripcion
            )
...
def eliminar_configuracion_global(request, id):
    if request.method == "POST":
        try:
            ...
            configuracion = get_object_or_404(
                ConfiguracionGeneralAuditoria, idconfiguracion=id
            )
            configuracion.delete()
            return JsonResponse({"success": True})
        except Exception as e:
