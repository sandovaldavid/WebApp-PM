...
from .models import Proyecto, Tarea, Requerimiento, Desarrollador, Tester, Administrador, Cliente
...
def dashboard(request):
    ...
    resumen_financiero = {
        "presupuesto_total": Proyecto.objects.aggregate(total=Coalesce(Sum('presupuesto', output_field=DecimalField()), 0, output_field=DecimalField()))['total'],
        "presupuesto_utilizado": Proyecto.objects.aggregate(utilizado=Coalesce(Sum('presupuestoutilizado', output_field=DecimalField()), 0, output_field=DecimalField()))['utilizado'],
        "presupuesto_restante": Proyecto.objects.aggregate(restante=Coalesce(Sum(F('presupuesto') - F('presupuestoutilizado'), output_field=DecimalField()), 0, output_field=DecimalField()))['restante'],
    }
