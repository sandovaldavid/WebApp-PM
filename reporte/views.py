...
def index(request):
    ...
    try:
        ...
        fecha_fin = parse_date(request.GET.get("fecha_fin"), default=timezone.now().date())
        fecha_inicio = parse_date(
            request.GET.get("fecha_inicio"), 
            default=(fecha_fin - timedelta(days=30)) if fecha_fin else timezone.now().date() - timedelta(days=30)
        )
        ...
        if fecha_inicio and fecha_fin and fecha_inicio > fecha_fin:
            messages.warning(
                request, 
                _("La fecha de inicio no puede ser posterior a la fecha de fin. Se han invertido las fechas.")
            )
            # Intercambiar fechas
        ...
        tareas = Tarea.objects.select_related(
            'idrequerimiento', 
            'idrequerimiento__idproyecto'
        )
        
        # Aplicar filtros de fecha (asegurando que son dates completos)
        if fecha_inicio:
            fecha_inicio_datetime = timezone.make_aware(datetime.combine(fecha_inicio, datetime.min.time()))
            tareas = tareas.filter(fechacreacion__gte=fecha_inicio_datetime)
        
        if fecha_fin:
            fecha_fin_datetime = timezone.make_aware(datetime.combine(fecha_fin, datetime.max.time()))
...
def prepare_context(
    tareas, tipo_reporte, fecha_inicio, fecha_fin, proyecto_id, proyecto_obj, estadisticas, request
):
    ...
    if tipo_reporte == "general" or not tipo_reporte:
        ...
        if Actividad.objects.filter(entidad_tipo='Tarea').exists():
            ...
            historial = Actividad.objects.select_related(
                'idusuario'
            ).filter(
                entidad_tipo='Tarea',
            ...
            if fecha_inicio and fecha_fin:
                ...
                historial = historial.filter(fechacreacion__range=(fecha_inicio_dt, fecha_fin_dt))
        ...
        context["using_actividad"] = 'idusuario' in dir(historial.first()) if historial.exists() else False
...
def calcular_estadisticas_recursos(tareas):
    ...
    tareas_recursos = Tarearecurso.objects.filter(
        idtarea__in=tarea_ids
    ).select_related(
        'idrecurso', 
    ...
    for tr in tareas_recursos:
        ...
        recursos_data[recurso_id]['horas_asignadas'] += duracion_estimada * (tr.cantidad or 1)
        recursos_data[recurso_id]['horas_utilizadas'] += duracion_actual * (tr.cantidad or 1)
    
    # Calcular eficiencias
    for recurso in recursos_data.values():
        recurso['eficiencia'] = round(
            (recurso['horas_utilizadas'] / recurso['horas_asignadas'] * 100) 
            if recurso['horas_asignadas'] > 0 else 100,
            2
        )
...
def calcular_estadisticas_costos(tareas):
    ...
    totales = tareas.aggregate(
        total_estimado=Coalesce(
            Sum("costoestimado"), 0, output_field=models.DecimalField(max_digits=15, decimal_places=2)
        ),
        total_actual=Coalesce(
            Sum("costoactual"), 0, output_field=models.DecimalField(max_digits=15, decimal_places=2)
        ),
    )
    ...
    presupuesto_restante = presupuesto_proyecto - total_actual if presupuesto_proyecto > 0 else 0
    porcentaje_restante = 100 - round((total_actual / presupuesto_proyecto * 100), 2) if presupuesto_proyecto > 0 else 0
    ...
    return {
        ...
        "porcentaje_utilizado": round(
            ...
            if presupuesto_proyecto > 0 else 
        ...
        "periodo": periodo_str,
        "proyecto": proyecto_relacionado.nombreproyecto if proyecto_relacionado else "Todos los proyectos",
        "periodos": [
            {
                "fecha": p["periodo"].strftime("%d/%m/%Y") if periodo_str == "diario" else p["periodo"].strftime("%b %Y"),
                "estimado": p["estimado"],
                "actual": p["actual"],
                "variacion": round(
                    (
...
def calcular_estadisticas_tareas(tareas):
    ...
    for proyecto in proyectos:
        ...
        proyecto['porcentaje_completado'] = round(
            ...
            if proyecto['total'] > 0 else 0, 
            1
        )
        
        # Calcular días restantes
        if proyecto['idrequerimiento__idproyecto__fechafin']:
            dias_restantes = (proyecto['idrequerimiento__idproyecto__fechafin'] - datetime.now().date()).days
    ...
    try:
        ...
        if Actividad.objects.filter(entidad_tipo='Tarea').exists():
            ...
            if tipo_filtro == "estado":
                historial = historial.filter(Q(accion__icontains='estado') | Q(nombre__icontains='estado'))
            elif tipo_filtro == "asignacion":
                historial = historial.filter(Q(accion__icontains='asignación') | Q(nombre__icontains='asign'))
            elif tipo_filtro == "recursos":
                historial = historial.filter(Q(accion__icontains='recurso') | Q(nombre__icontains='recurso'))
            ...
            historial = Historialtarea.objects.select_related(
                ...
                idtarea__isnull=False  # Filtrar entradas con tareas nulas
            ).order_by("-fechacambio")
        ...
        context = {
            ...
            "using_actividad": 'idusuario' in dir(historial.first()) if historial.exists() else False
        }
    ...
    try:
        ...
        if fecha_inicio_str and fecha_fin_str:
            try:
                ...
                tareas = tareas.filter(fechacreacion__range=(fecha_inicio_dt, fecha_fin_dt))
        ...
        writer.writerow([f"Fecha de generación: {timezone.now().strftime('%d/%m/%Y %H:%M:%S')}"])
        writer.writerow([f"Período: {fecha_inicio_str or 'No especificado'} a {fecha_fin_str or 'No especificado'}"])
        ...
        if tipo_reporte == "recursos":
            ...
            for recurso in datos_recursos["recursos"]:
                ...
                for tr in Tarearecurso.objects.filter(
                    idtarea__in=tareas,
                    idrecurso__nombrerecurso=recurso["nombre"]
                ...
                writer.writerow([
                    recurso["nombre"],
                    recurso["tipo"],
                    ", ".join(tareas_recurso)[:100] + ("..." if len(", ".join(tareas_recurso)) > 100 else ""),
            ...
            writer.writerow([
                ...
                f"{(datos_recursos['total_horas_utilizadas'] / datos_recursos['total_horas_asignadas'] * 100):.2f}%" if datos_recursos['total_horas_asignadas'] else "0%",
                "",
                ""
            ])
            ...
            writer.writerow([
                ...
                "SOBRE PRESUPUESTO" if datos_costos['variacion_total'] > 0 else "BAJO PRESUPUESTO"
            ])
            writer.writerow([])
            writer.writerow([
            ...
            for tarea in tareas:
                ...
                writer.writerow([
                    ...
                    tarea.idrequerimiento.descripcion[:50] + ("..." if len(tarea.idrequerimiento.descripcion) > 50 else ""),
                    tarea.estado,
                    tarea.duracionestimada or 0,
                    tarea.duracionactual or 0,
                    f"${tarea.costoestimado or 0:.2f}",
                    f"${tarea.costoactual or 0:.2f}",
                    tarea.fechainicio.strftime("%d/%m/%Y") if tarea.fechainicio else "No definida",
                    tarea.fechafin.strftime("%d/%m/%Y") if tarea.fechafin else "No definida",
                    tarea.tipo_tarea.nombre if tarea.tipo_tarea else "No especificado",
            ...
            writer.writerow(["Completadas", completadas, f"{(completadas/total_tareas*100) if total_tareas else 0:.1f}%"])
            writer.writerow(["En progreso", en_progreso, f"{(en_progreso/total_tareas*100) if total_tareas else 0:.1f}%"])
            writer.writerow(["Pendientes", pendientes, f"{(pendientes/total_tareas*100) if total_tareas else 0:.1f}%"])
            
            # Agregar datos de costos totales
            total_estimado = tareas.aggregate(Sum('costoestimado'))['costoestimado__sum'] or 0
            ...
            writer.writerow(["Variación", f"${total_actual - total_estimado:.2f}", 
    ...
    try:
        ...
        if tipo_reporte == "recursos":
            ...
            datos_especificos["datos_tareas"] = {
                "tareas_por_tipo": tareas.values('tipo_tarea__nombre').annotate(
                    count=Count('idtarea')
                ).order_by('-count'),
                "tareas_por_fase": tareas.values('fase__nombre').annotate(
                    count=Count('idtarea')
        ...
        context_pdf = {
            ...
            "filtros": {
                "fecha_inicio": fecha_inicio,
                "fecha_fin": fecha_fin,
                "proyecto": proyecto_obj if 'proyecto_obj' in locals() and proyecto_obj else None,
                "tipo_reporte": tipo_reporte
            },
            "request": request,
            **datos_especificos
        ...
        timestamp = timezone.now().strftime("%Y%m%d_%H%M%S")
        proyecto_nombre = proyecto_obj.nombreproyecto.replace(" ", "_") if 'proyecto_obj' in locals() and proyecto_obj else "todos"
